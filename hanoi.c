...
int main(void) {
  ...
  while( tower[3][2] != DISKS){
    int car = getch();

    if(car == KEY_QUIT) break;
    
    else if(car == KEY_K || car == KEY_L || car == KEY_M){
      if(!play){
	play = car ;
	continue;
      }

      else if(car == play)continue;
      
      swap(play-KEY_TO_ZERO, car-KEY_TO_ZERO, tower, dbg);
      clearTower(main_win);
      drawTour(main_win, tower);
      dbg_update(dbg, tower);
      wrefresh(dbg);
  ...
  if(tower[3][2] == DISKS){
    mvwprintw( main_win, LINES/2, RCOLS / 2 ,"GAGNE");
    wrefresh(main_win);
    getch();
  }
